...
import {addEvent, bindAttribute, bindClassName, bindStyle, virtualNode} from './internal/dom';
import {noop} from './internal/func';
import {isStore} from './stores';
...
export interface AttributesDirectiveProps {
	 ...
	 */
	events?: Partial<Record<keyof HTMLElementEventMap, (event: any) => void>>;
...
const getStore = <T>(value: T | ReadableSignal<T>) => (isStore(value) ? value : writable(value));
...
export const createAttributesDirective =
	...
	(node: HTMLElement, args: T) => {
		...
		for (const [type, eventFn] of Object.entries(events ?? {})) {
			unsubscribers.push(addEvent(node, type as keyof HTMLElementEventMap, eventFn));
		}

		for (const [attributeName, value] of Object.entries(attributes ?? {})) {
			if (value != null) {
				unsubscribers.push(bindAttribute(node, attributeName, getStore(value)));
			}
		}

		for (const [styleName, value] of Object.entries(styles ?? {})) {
			if (value) {
				unsubscribers.push(bindStyle(node, styleName, getStore(value)));
			}
		}

		for (const [className, value] of Object.entries(classNames ?? {})) {
			unsubscribers.push(bindClassName(node, className, getStore(value)));
		}
...
/**
 * Retrieves the attributes generated by the directive.
 * Useful for creating appropriate markup during server-side rendering (SSR).
 *
 * @param directive - The directive to be processed.
 * @param args - Optional arguments required by the directive, if any.
 * @returns An object representing attribute names and their corresponding values.
 *
 * @remarks
 * This function processes the directive with a virtual node.
 * The virtual node includes mock functions such as addEventListener, removeEventListener, setAttribute, removeAttributes, classList
 * and contains the 'style' object. Utilizing additional node features may result in errors.
 */
export function directiveAttributes<T = void>(directive: Directive<T>, args?: T) {
...
/**
 * Returns the attributes given by {@link directiveAttributes}, but only in a SSR environement,
 ...
 *
 * @remarks
 * When executed server-side, this function processes the directive with a virtual node.
 * The virtual node includes mock functions such as addEventListener, removeEventListener, setAttribute, removeAttributes,
 * and contains the 'style' object. Utilizing additional node features may result in errors.
 */
export function ssrDirectiveAttributes<T = void>(directive: Directive<T>, args?: T) {
	return BROWSER ? {} : directiveAttributes(directive, args);
}
